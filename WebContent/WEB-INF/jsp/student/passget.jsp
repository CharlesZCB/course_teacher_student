...
<div class="login">
	<h1>密码重置</h1>
	<form>
		<input type="text" id="scode" placeholder="学号" required="required" id="name" autocomplete="off"/>
		...
		<button type="button" class="btn btn-primary btn-block btn-large" onclick="subnewpass()">确认修改</button>
	</form>
	...
	if(scode!=codes){
		...
		if(pass1==pass2){
			$.ajax({
				...
				dataType:"text",
				type:"post",
				url:"${path}/users/editpass2",
				success:function(msg){
					if(msg=="ok"){
						alert("密码修改成功！");
						window.location.href="${path}/front/loginstudent"
					}
				}
